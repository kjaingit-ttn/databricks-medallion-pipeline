#!/usr/bin/env python3
"""Create final Silver tables by orchestrating all four quality-check categories.

This module reads Bronze tables, applies Completeness, Uniqueness, Referential Integrity,
and Business Logic checks inline, and writes all rows (PASS + FAIL) to final Silver tables.
"""
...
def get_spark() -> SparkSession:
    """Return active Spark session (Databricks) or create one locally."""
...
def drop_silver_output_tables(spark: SparkSession) -> None:
...
def build_table_report(spark: SparkSession, table_name: str, df: DataFrame) -> DataFrame:
...
def run_create_silver_tables(spark: SparkSession) -> DataFrame:
    ...
    customers_report = build_table_report(spark, "customers", customers_silver)
    orders_report = build_table_report(spark, "orders", orders_silver)
    products_report = build_table_report(spark, "products", products_silver)
...
def main() -> None:
    spark = get_spark()
    run_create_silver_tables(spark)
