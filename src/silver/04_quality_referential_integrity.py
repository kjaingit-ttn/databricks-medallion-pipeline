#!/usr/bin/env python3
"""Silver referential-integrity quality checks for orders.

Referential-integrity checks add boolean flag columns without dropping rows. Null or blank
foreign keys are treated as PASS in this check because completeness rules own missing-key
validation; this check only catches non-null/non-blank orphan references.
"""
...
def report_check(df: DataFrame, check_col: str, label: str) -> dict[str, int | float]:
...
def verify_expected_orphans(reports: dict[str, dict[str, int | float]]) -> None:
...
def run_referential_integrity_checks(spark: SparkSession) -> dict[str, dict[str, int | float]]:
...
def main() -> None:
    spark = get_spark()
    run_referential_integrity_checks(spark)
