#!/usr/bin/env python3
"""Silver business-logic quality checks for orders and customers.

Business-logic checks are domain rules layered on top of structural checks
(completeness, uniqueness, referential integrity). These checks only add boolean
flags and never drop Bronze rows.
"""
...
def run_business_logic_checks(spark: SparkSession) -> dict[str, dict[str, int | float]]:
...
def main() -> None:
    spark = get_spark()
    run_business_logic_checks(spark)
